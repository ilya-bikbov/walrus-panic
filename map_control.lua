...
local function box_move(direction, box_x, box_y)
    ...
    if direction == "left" then
        box_next_block_type = get_block_type {
            x = box_x - 1,
            y = box_y
        }
    ...
    if box_next_block_type == "1" then
        level_matrix[box_y][box_x], level_matrix[next_coord_y][next_coord_x] = level_matrix[next_coord_y][next_coord_x],
            level_matrix[box_y][box_x]
        return true
    end
    return false
end
...
function player_move(direction)
    ...
    if direction == "left" then
        next_block_type = get_block_type {
            x = player.x - 1,
            y = player.y
        }
        next_coord_x = next_coord_x - 1
    ...
    if next_block_type == "1" then
        ...
        level_matrix[player.y][player.x], level_matrix[next_coord_y][next_coord_x] =
            level_matrix[next_coord_y][next_coord_x], level_matrix[player.y][player.x]
        -- print("^ swap v")
        -- print (level_matrix[player.y][player.x])
    end
